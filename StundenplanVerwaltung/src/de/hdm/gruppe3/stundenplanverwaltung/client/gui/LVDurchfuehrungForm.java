...
public class LVDurchfuehrungForm extends VerticalPanel {
		...
		stundenplanVerwaltung
				.getAllLV(new AsyncCallback<Vector<Lehrveranstaltung>>() {
					...
					public void onSuccess(Vector<Lehrveranstaltung> result) {
						...
						// Erster Eintrag soll leer sein bzw. Bitte wählen
						// enthalten
						...
						for (Lehrveranstaltung lv : result) {
							...
							// Lehrveranstatlungs Id welche beim anlegen der
							// Durchführung
							// benötigt wird.
							lbLehrveranstaltung.addItem(lv.getBezeichnung(),
									String.valueOf(lv.getId()));
						}

						// Listbox setzen wenn im editier Modus
						if (selectedLVDurchfuehrung != null) {
							int lehrveranstaltungId = selectedLVDurchfuehrung
									.getLehrveranstaltung().getId();
							selectListenAuswahl(lbLehrveranstaltung,
									lehrveranstaltungId);
						}
	...
	private void setRaumListBox() {
		stundenplanVerwaltung.getAllRaeume(new AsyncCallback<Vector<Raum>>() {
			...
			public void onSuccess(Vector<Raum> result) {
				...
				for (Raum r : result) {
					// Der zweite Parameter von addItem ist die gew�hlte Raum Id
					// welche beim anlegen der Durchführung
					// v wird.
					lbRaum.addItem(r.getBezeichnung(),
				...
				.getAllSemesterverband(new AsyncCallback<Vector<Semesterverband>>() {
					...
					public void onSuccess(Vector<Semesterverband> result) {
						...
						for (Semesterverband sv : result) {
							// Der erste Prameter von addItem enthält den
							// anzeige String, er wird in der toString methode
							// zu einem lesbaren String umgewandelt.
							// Der zweite Parameter von addItem ist die gewählte
							// Semesterverband Id welche beim anlegen der
							// Durchführung
							// benögtigt wird.
							lbSemesterverband.addItem(sv.toString(),
									String.valueOf(sv.getId()));
						}

						// Listbox setzen wenn im editier Modus
						if (selectedLVDurchfuehrung != null) {
							int semesterverbandId = selectedLVDurchfuehrung
									.getSemesterverband().getId();
							selectListenAuswahl(lbSemesterverband,
									semesterverbandId);
						}
	...
	private void setZeitListBox() {
		...
		if (selectedLVDurchfuehrung != null) {
			int anfangZeit = selectedLVDurchfuehrung.getZeitslot()
					.getAnfangszeit();
	...
	private void setWochentagListBox() {
		...
		// Läuft durch den ganzen Wochentag Array der in ConstantsStdpln
		// definiert ist
		...
		if (selectedLVDurchfuehrung != null) {
			String wochentag = selectedLVDurchfuehrung.getZeitslot()
					.getWochentag();
	...
	/**
	 * Setzt beim editieren den richtigen Eintrag. In GWT können die Felder nur
	 * nach einem Index gesetzt werden. Der Index unterscheidet sich von der
	 * Value der HTML SelectBox welches die Id des gewählten Business Objektes
	 * ist. Diese Objekt Id wird beim modifizieren benötigt damit sie mit dem
	 * Datensatz in der Datenbank in verbindung gebracht werden kann. Id ist
	 * gleich Primärschlüssel.
	 * 
	 * @param listBox
	 *            : Die zu setzende Listbox
	 * @param zuSetzendeValue
	 *            : die Value die ausgewählt werden soll
	 */
	private void selectListenAuswahl(ListBox listBox, int zuSetzendeValue) {
		// Nur ausführen wenn selected Objekt gefüllt ist und die Objekt Id
		// vorhanden ist, sonst gibt es Fehler.
		if (selectedLVDurchfuehrung != null && zuSetzendeValue > 0) {
			for (int pos = 0; pos < listBox.getItemCount(); pos++) {
				...
				if (currentValue == zuSetzendeValue) {
					// Wenn die Vaulue gleich die zuSetzendeValue ist muss der
					// listbox eintrag an der aktuellen Position gewählt werden.
	...
	/**
	 * Das gleiche wie {@link #selectListenAuswahl(ListBox, int)}, nur für die
	 * Zeit und Wochentage. Da dort Strings und keine Integer als Value
	 * verwendet werden ist eine extra Methode nötig.
	 * 
	 * @param listBox
	 * @param zuSetzendeValue
	 */
		...
		if (selectedLVDurchfuehrung != null && zuSetzendeValue != null) {
			for (int listBoxPos = 0; listBoxPos < listBox.getItemCount(); listBoxPos++) {
				...
				if (currentValue.equals(zuSetzendeValue)) {
					// Wenn die Vaulue gleich zuSetzendeValuet Id ist muss der
					// listbox eintrag an der aktuellen Position gewählt werden.
	...
	public void showButtons() {
		// Nur wenn Business Objekt geändert wird, dann werden der modifizieren
		// und
	...
	public void setFields() {
		// richten Eintrag in den Eingabefeldern wählen wenn eine
		// Lehrveranstaltung existiert
		if (selectedLVDurchfuehrung != null) {
	...
	public void modifizierenSelectedLVDurchfuehrung() {
		...
		int anfangsZeit = Integer.valueOf(lbAnfangszeit.getValue(lbAnfangszeit
				.getSelectedIndex()));
		int endZeit = Integer.valueOf(lbEndzeit.getValue(lbEndzeit
				.getSelectedIndex()));
		String wochentag = lbWochentag.getItemText(lbWochentag
				.getSelectedIndex());

		// Zeitslot Objekt mit aus der ListBox gelesenen Werten f�llen, die id
		// fehlt und wird in den mapper emittelt.
		...
		int svId = Integer.valueOf(lbSemesterverband.getValue(lbSemesterverband
				.getSelectedIndex()));

		// Wert f�r Raum auslsen
		int raumId = Integer
				.valueOf(lbRaum.getValue(lbRaum.getSelectedIndex()));

		// Wert f�r Lehrveranstaltung auslsen
		int lvId = Integer.valueOf(lbLehrveranstaltung
				.getValue(lbLehrveranstaltung.getSelectedIndex()));

		// Ruft Serverseitige Methode auf
		stundenplanVerwaltung.modifizierenDurchfuehrung(lvdId, svId, raumId,
				lvId, zeitslot, new AsyncCallback<LVDurchfuehrung>() {
					...
					public void onFailure(Throwable caught) {
						...
						if (caught instanceof RaumBelegtException) {
							message = ((RaumBelegtException) caught)
									.getFehlerMessage();
						}
	...
	public void loeschenSelectedLVDurchfuehrung() {
		if (this.selectedLVDurchfuehrung != null) {
			...
			stundenplanVerwaltung.loeschenDurchfuehrung(
					...
					new AsyncCallback<LVDurchfuehrung>() {
	...
	public void anlegenLVDurchfuehrung() {
		...
		// Ruft Serverseitige Methode auf
		stundenplanVerwaltung.anlegenDurchfuehrung(svId, raumId, lvId,
				zeitslot, new AsyncCallback<LVDurchfuehrung>() {
	...
	private boolean validiereBenutzerEingabe() {
		...
		if (indexLV < 1 || indexRaum < 1 || indexSV < 1 || indexAnfangsZeit < 1
